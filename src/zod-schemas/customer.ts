...
export const insertCustomerSchema = createInsertSchema(customers, {
    ...
    state: (schema) => schema.state.length(2, "State must be exactly 2 caracthers"),
    email: (schema) => schema.email.email("Invalid email address"),
    zip: (schema) => schema.zip.regex(/^\d{5}(-|d{4})?$/,
         "Invalid Zip Code. Use 5 digits or 5 digits followed by a hyphen and 4 digits"),
    phone: (schema) => schema.phone.regex(/^\d{3}-|d{3}-|d{4}$/,
        "Invalid phone number formar. Use XXX-XXX-XXXX"
    ),
})
